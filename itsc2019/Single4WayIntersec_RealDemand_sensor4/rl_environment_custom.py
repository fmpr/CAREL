...
class RLEnvironmentCustom(RLEnvironment):
	...
	def __init__(self, scenario_path, config_file, env_name='env'):
		self.sensor_failure = [[False, False], 
	...
	def get_data_for_controller(self, junct_id):
		...
		raw_inputs = [[] for i in range(4)]
		raw_inputs[0] = [np.mean(np.array(self.tl_queue_lenghts[913][861][0])[window]), np.mean(np.array(self.tl_queue_lenghts[913][865][0])[window])]
		raw_inputs[1] = [np.mean(np.array(self.tl_queue_lenghts[913][861][1])[window]), np.mean(np.array(self.tl_queue_lenghts[913][865][1])[window]), 
		...
		raw_inputs[2] = [np.mean(np.array(self.tl_queue_lenghts[913][863][0])[window]), np.mean(np.array(self.tl_queue_lenghts[913][867][0])[window])]
		raw_inputs[3] = [np.mean(np.array(self.tl_queue_lenghts[913][863][1])[window]), np.mean(np.array(self.tl_queue_lenghts[913][867][1])[window]),
		...
		for s in range(len(self.sensor_failure)):
			for i in range(len(self.sensor_failure[s])):
				if self.sensor_failure[s][i]:
					if np.random.rand() < 0.005: # failure recover probability
						self.sensor_failure[s][i] = False
				else:
					if np.random.rand() < 0.001: # failure probability
						self.sensor_failure[s][i] = True

				if self.sensor_failure[s][i]:
					raw_inputs[s][i] = 0.0

		# input aggregation with max()
		env_inputs = np.zeros(4)
		for i in range(len(env_inputs)):
			env_inputs[i] = np.max(raw_inputs[i])
